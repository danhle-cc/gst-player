...
#if 0
    g_print ("duration=%f\n", duration / ((double) 1000 * 1000 * 1000)); // print seconds
    //g_print ("position=%llu\n", pos);
    g_print ("position=%f\n", pos / ((double) 1000 * 1000 * 1000)); // print seconds
#endif
...
{
    gtk_init (&argc, &argv);
    backend_init (&argc, &argv);	// all this does is call gst_init()  :(

    start ();		// this biulds and shows the window
